...
void Rasterizer::InitPrograms() {
  ...
  shadowVolumeProgram =
      new ShaderProgram("../shaders/vertex/ShadowVolume.vert",
                        "../shaders/geometry/ShadowVolume.geom",
                        "../shaders/fragment/ShadowVolume.frag");
  shadowDarkenProgram =
      new ShaderProgram("../shaders/vertex/ShadowDarken.vert",
...
void Rasterizer::RenderDepthPass() {
  ...
  lightSpaceMatrix = CalculateTightLightSpaceMatrix();
...
void Rasterizer::RenderStencilShadowPass(const glm::mat4 &viewProjection) {
  ...
  glUniformMatrix4fv(glGetUniformLocation(shadowVolumeProgram->getProgramID(),
                                          "viewProjection"),
                     1, GL_FALSE, &viewProjection[0][0]);
  glUniform3f(glGetUniformLocation(shadowVolumeProgram->getProgramID(),
                                   "lightDirection"),
              animatedLightDirection.x, animatedLightDirection.y,
              animatedLightDirection.z);
  glUniform1f(glGetUniformLocation(shadowVolumeProgram->getProgramID(),
                                   "extrusionDistance"),
              shadowVolumeExtrusion);
  glUniform1i(
      glGetUniformLocation(shadowVolumeProgram->getProgramID(), "invertFacing"),
...
void Rasterizer::RenderShadowDarkenPass() {
  ...
  glUniform1f(glGetUniformLocation(shadowDarkenProgram->getProgramID(),
                                   "shadowOpacity"),
...
void Rasterizer::RenderStencilDebugPass() {
  ...
  glUniform1f(
      glGetUniformLocation(stencilDebugProgram->getProgramID(), "debugOpacity"),
...
void Rasterizer::MainLoop() {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    auto setupMainProgram = [&](bool enableShadowMap, int lightCount,
      ...
      glUniform1i(
          glGetUniformLocation(program->getProgramID(), "irradianceMap"), 14);

      glActiveTexture(GL_TEXTURE15);
      glBindTexture(GL_TEXTURE_2D, prefilteredMap);
      glUniform1i(
          glGetUniformLocation(program->getProgramID(), "prefilteredMap"), 15);
      ...
      glUniform1f(
          glGetUniformLocation(program->getProgramID(), "shadowBiasMin"),
          shadowBiasMin);
      glUniform1f(
          glGetUniformLocation(program->getProgramID(), "shadowBiasMax"),
          shadowBiasMax);
      glUniform1f(
          glGetUniformLocation(program->getProgramID(), "uAmbientScale"),
          ambientScale);
...
glm::mat4 Rasterizer::CalculateTightLightSpaceMatrix() {
  ...
  float shadowDistance = std::min(originalFar, 10.f);
  camera->setZFar(shadowDistance);

  glm::mat4 camProj = camera->getProjectionMatrix(ratio);
  ...
  glm::vec3 lightPos = center - animatedLightDirection;
