...
static void _read_directory_entry(FatFileSystem *self, uint32_t dir_cluster)
{
    if (!self->selected_partition_table)
    {
        printf("No partition table selected\n");
...
static int _find_file(FatFileSystem *self, uint32_t dir_cluster, const char *name, const char *ext, Fat16Entry *out_entry)
{
    /* 
    ...
    for (int i = 0; i < 8 && name[i] != '\0'; i++) target_name[i] = name[i];
    for (int i = 0; i < 3 && ext[i] != '\0'; i++) target_ext[i] = ext[i];
    ...
    {
        _read_directory_entry(self, dir_cluster);
        ...
        int match = 1;
        for (int j = 0; j < 8; j++) if (entry.filename[j] != target_name[j]) match = 0;
        for (int j = 0; j < 3; j++) if (entry.ext[j] != target_ext[j]) match = 0;
...
static void _read_file(FatFileSystem *self, Fat16Entry *entry, FILE *out_stream)
{
    if (!entry || entry->starting_cluster < 2) return;
