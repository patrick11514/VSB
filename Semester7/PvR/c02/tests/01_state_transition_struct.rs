...
fn pc_transition(computer: ComputerState, event: Event) -> ComputerState {
    match event {
        Event::TurnOn => ComputerState {
            is_running: true,
            is_sleeping: computer.is_sleeping,
            idle_time: computer.idle_time,
            sleep_time: computer.sleep_time,
        ...
        Event::PassTime(time) => {
            ...
            if computer.is_sleeping() {
                new_state = ComputerState {
                    is_running: true,
                    is_sleeping: true,
                    uptime: computer.uptime + time,
                    idle_time: 0,
                    sleep_time: computer.sleep_time + time,
                };
            } else {
                new_state = ComputerState {
                    is_running: computer.is_running,
                    is_sleeping: computer.is_sleeping,
                    uptime: computer.uptime + time,
                    idle_time: computer.idle_time + time,
                    sleep_time: computer.sleep_time,
                };

                if new_state.idle_time >= 1000 {
                    new_state.is_sleeping = true;
                    new_state.sleep_time = new_state.idle_time - 1000;
                    new_state.idle_time = 0;
                }
            }

            if new_state.sleep_time > 500 {
