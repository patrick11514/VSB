//! Run this file with `cargo test --test 03_adjacent_diff`.
...
fn adjacent_diff(slice: &[i32]) -> impl Iterator<Item = i32> {
    //https://stackoverflow.com/a/66386280
    slice.windows(2).map(|slice| slice[1] - slice[0])
}
