//! Run this file with `cargo test --test 02_cumulative_sum`.
...
where
    T: Iterator,
    T::Item: Add<Output = T::Item> + Copy,
{
    iter: T,
    prev: Option<T::Item>,
}

impl<T: Iterator> CumulativeSum<T>
where
    T::Item: Add<Output = T::Item> + Copy,
{
    fn new(iter: T) -> Self {
        CumulativeSum { iter, prev: None }
    }
}

impl<T: Iterator> Iterator for CumulativeSum<T>
where
    T::Item: Add<Output = T::Item> + Copy,
{
    type Item = T::Item;
