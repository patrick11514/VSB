//! Run this file with `cargo test --test 01_split_items`.
...
impl<'a> SplitItems<'a> {
    fn new(str: &'a str, del: char) -> Self {
        SplitItems {
            str,
            del,
            idx: 0,
...
impl<'a> Iterator for SplitItems<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            println!("idx: {} len: {}", self.idx, self.len);
            if self.idx >= self.len {
                return None;
            }

            let idx = match self.str[self.idx..].find(self.del) {
                Some(idx) => self.idx + idx,
                None => self.len,
            };

            let result = &self.str[self.idx..idx];
            self.idx = idx + 1;
