//! Run this file with `cargo test --test 06_parser_combinator`.

//! TODO: Implement a JSON parser using parser combinators.
//! Good news: I wrote the parser for you.
//! Bad news: You have to implement the parser combinators (the LEGO building blocks that were used
...
mod tests {
    ...
    fn test_sequence() {
        ...
        check(
            sequence(char_parser('a'), char_parser('b')),
            "abc",
            "c",
            ('a', 'b'),
        );
        check(
            sequence(string_parser("hello"), char_parser('!')),
            "hello!world",
            "world",
            (String::from("hello"), '!'),
        );
        check(
            sequence(take_while(|c| c.is_ascii_digit()), char_parser('a')),
            "123a4",
            "4",
            (String::from("123"), 'a'),
        );
        check(
            sequence(take_while(|c| c.is_ascii_alphabetic()), char_parser('!')),
            "abc!xyz",
            "xyz",
            (String::from("abc"), '!'),
        );
        check(
            sequence(char_parser('x'), take_while(|c| c.is_ascii_lowercase())),
            "xhello123",
            "123",
            ('x', String::from("hello")),
        );
        ...
        check(
            ...
            "HELLO123abc",
            "abc",
            (String::from("HELLO"), String::from("123")),
        );
    }
    ...
    fn test_repeated() {
        ...
        check(
            repeated(map(char_parser('1'), |c| c.to_string())),
            "1112",
            "2",
            vec![String::from("1"), String::from("1"), String::from("1")],
        );
        check(
            repeated(delimited_by('[', ']', char_parser('x'))),
            "[x][x]a",
            "a",
            vec!['x', 'x'],
        );
        check(
            repeated(followed_by(char_parser('a'), char_parser('b'))),
            "ababac",
            "ac",
            vec!['a', 'a'],
        );
        check(
            repeated(map(char_parser('1'), |_| 1)),
            "111x",
            "x",
            vec![1, 1, 1],
        );
        check(
            ...
            "(12)(34)A",
            "A",
            vec![String::from("12"), String::from("34")],
        );
    }
