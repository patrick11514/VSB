...
//! TODO (8p): implement a simple chat server using non-blocking I/O
//!
...
//! an arbitrary duration of time, it should always sleep until the next I/O event or the next
//! timeout event. You can use `epoll` on Linux or the `mio` crate on macOS/Windows (or any other
//! approach that you want) to achieve this.
//!
...
use crate::running_server::RunningServer;
...
mod tests {
    ...
    use crate::{RunningServer, ServerOpts, run_server};
    ...
    fn join_timeout() {
        run_test(opts(2), |server| {
            let mut client = server.client();
            sleep(3000);
            if let Ok(_) = client.try_send(ClientToServerMsg::Join {
