...
//!
//! The solution should fit within 200-300 lines of code.
//!
...
mod tests {
    ...
    fn axe_maker() {
        ...
        let (factorio, tx, rx) = FactorioBuilder::new(4)
            ...
            .fork_join(
                |ore, furnace_id| {
                    let amount = rand::rng().random_range((furnace_id * 2) as u64..ore.0.weight);
                    std::thread::sleep(Duration::from_millis(rand::rng().random_range(1..3)));
        ...
        let handle = std::thread::spawn(move || {
            for _ in 0..1000 {
                let ore_amount = rand::rng().random_range(10..150);
