//! Run this file with `cargo test --test 04_state_transition`.
...
fn pc_transition(state: ComputerState, event: Event) -> ComputerState {
    match (event, &state) {
        (Event::TurnOn, ComputerState::Running { .. } | ComputerState::Sleeping { .. }) => state,
        ...
        (Event::MoveMouse, ComputerState::Off) => ComputerState::Off,
        (
            Event::MoveMouse,
        ...
        (Event::PassTime(_), ComputerState::Off) => ComputerState::Off,
        (Event::PassTime(time), state) => {
            let mut new_state: ComputerState;
            if let ComputerState::Running { uptime, idle_time } = state {
                let mut new_idle_time = *idle_time + time;
                let mut local_state = ComputerState::Running {
                    uptime: uptime + time,
            ...
            } else if let ComputerState::Sleeping { uptime, sleep_time } = state {
                new_state = ComputerState::Sleeping {
                    uptime: *uptime + time,
            ...
            if let ComputerState::Sleeping { sleep_time, .. } = new_state
                && sleep_time > 500
            {
                new_state = ComputerState::Off
            }
