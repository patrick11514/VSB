...
void outputSequence(int sock_server) {
  ...
  int ret = write(sock_server, sleeping, strlen(sleeping));
  if (ret < 0) {
...
int main(int t_narg, char **t_args) {
  ...
  if (t_narg >= 4) {
    ...
    int writed = write(l_sock_server, buffer, strlen(buffer));
    if (writed <= 0) {
      printf("Unable to send nick to server\n");
    }
  }
  ...
  while (1) {
    ...
    // select from fds
    if (poll(l_read_poll, 2, timeout) < 0)
      break;
