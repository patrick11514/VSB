...
void childHandling(int l_sock_client) {
  while (1) { // communication
    ...
    char buff[100];
    sprintf(buff, "%d.srv", getpid());

    if (fork() == 0) {
      close(l_sock_client);

      execlp("g++", "g++", "../hello.cpp", "-D", l_buf, "-o", buff, nullptr);
      return;
    }

    wait(NULL);

    int binary = open(buff, O_RDONLY);

    if (binary < 0) {
      log_msg(LOG_ERROR, "Cannot open builded file");
      close(l_sock_client);
      return;
    }
    ...
    while (int readed = read(binary, buffer, sizeof(buffer))) {
      write(l_sock_client, buffer, readed);
      usleep(1000);
    }

    printf("Readed all, closing\n");
