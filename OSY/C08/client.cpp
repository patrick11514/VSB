...
void help(int t_narg, char **t_args) {
  if (t_narg <= 3 || !strcmp(t_args[1], "-h")) {
    printf("\n"
           ...
           "\n"
           "  Use: %s [-h -d] ip_or_name port_number image\n"
           "\n"
           "    -d  debug mode \n"
           "    -h  this help\n"
           "\n",
...
int main(int t_narg, char **t_args) {
  ...
  char buffer[255];
  sprintf(buffer, "#img %s\n", t_args[3]);
  int ret = write(l_sock_server, buffer, strlen(buffer));
  if (ret <= 0) {
    log_msg(LOG_ERROR, "Unable to write image");
    return EXIT_FAILURE;
  }

  char fileName[255];
  sprintf(fileName, "%d.img", getpid());
  ...
  if (fork() == 0) {
    execlp("display", "display", fileName, nullptr);
    return 0;
  }
