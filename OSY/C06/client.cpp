...
void outputSequence(int sock_server) {
  int nums = std::floor((std::rand() % 20));
  char buffer[512] = {0};
  ...
  for (int i = 0; i < nums; ++i) {
    ...
    if (i == nums - 1) {
      ...
      sprintf(buffer, "%s%d%c", buffer, num,
              getChar(std::floor(std::rand() % 2)));
    }
  }
  int ret = write(sock_server, buffer, strlen(buffer));
  if (ret < 0) {
    log_msg(LOG_ERROR, "Unable to write to server");
  }
}
...
int main(int t_narg, char **t_args) {
  ...
  // int timeout = 15 * 1000; // 15s

  char buffer[255];

  sprintf(buffer, "%s\n", t_args[3]);

  int l_len = write(l_sock_server, buffer, strlen(buffer));
  if (l_len < 0)
  ...
  char buff[255];
  sprintf(buff, "%d.cl", getpid());

  int binary = open(buff, O_WRONLY | O_CREAT, 0777);
  ...
  while (1) {
    ...
    /*else if (!(l_read_poll[1].revents & POLLIN)) {
  // no data from keybord, so generate it
  outputSequence(l_sock_server);
}*/
    ...
    // write to file
    l_len = write(binary, l_buf, l_len);
    if (l_len < 0)
      log_msg(LOG_ERROR, "Unable to write to binary file.");
