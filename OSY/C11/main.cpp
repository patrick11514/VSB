...
void handle_btn(int server, int id, int state) {
  int time = id + 1;

  if (state == 1) {
    char buffer[255];
    sprintf(buffer, "OPEN %d\n", time);
    write(server, buffer, strlen(buffer));
    printf("sending open\n");

    std::this_thread::sleep_for(std::chrono::seconds(time + 1));

    sprintf(buffer, "CLOSE %d\n", time);
    write(server, buffer, strlen(buffer));
...
int main(int t_narg, char **t_args) {
  ...
  while (1) {
    ...
    if (l_read_poll[1].revents & POLLIN) {
      ...
      // handle buttons
      if (strncmp(l_buf, "BTN", 3) == 0) {
        // handle buttons
        int id;
        int state;

        sscanf(l_buf, "BTN%d %d\n", &id, &state);

        std::thread t(handle_btn, l_sock_server, id, state);
        t.detach();
      }
      //  request to close?
