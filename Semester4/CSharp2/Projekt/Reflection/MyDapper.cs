...
public class MyDapper : IDisposable, IAsyncDisposable
{
    ...
    public async Task<IEnumerable<T>> Select<T>(string expression)
    {
        var type = typeof(T);
        //get primary key
        PropertyInfo? pkProperty = GetPrimaryProperty(typeof(T));
        ...
        StringBuilder sb = new();
        sb.Append($"SELECT * FROM {type.Name} WHERE {expression}");
    ...
    public async Task Delete<T>(Dictionary<string, object> search)
    {
        var type = typeof(T);
        var properties = type.GetProperties();
