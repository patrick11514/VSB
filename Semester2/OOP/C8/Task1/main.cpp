...
void printData(int accounts, Bank *bank)
{
    ...
    {
        Account *account = (PartnerAccount *)bank->GetAccount(i);

        Client *owner = account->GetOwner();
        Client *partner = nullptr;

        if (account->getType() == AccountType::Partner)
        {
            partner = ((PartnerAccount *)account)->GetPartner();
        }
        ...
        cout << account->GetNumber()
             ...
             << account->GetInterest()
             << "\t Vybratelné: "
             << (account->getType() == AccountType::Normal    ? account->CanWithdraw(100)
                 : account->getType() == AccountType::Partner ? ((PartnerAccount *)account)->CanWithdraw(100)
                                                              : ((CreditAccount *)account)->CanWithdraw(100))
             << "\t Typ:      "
             << account->getType()
             << endl;
    }
...
int main()
{
    ...
    for (int i = 0; i < accounts; i++)
    {
        ...
        {
            if (i % 2 == 0)
                account = bank->CreateCreditAccount(number, client, ir, 500);
            else
