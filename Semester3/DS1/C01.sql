...
SELECT payment_id, DAY(payment_date) Den, MONTH(payment_date) Mesic, YEAR(payment_date) Rok
FROM payment;
...
SELECT customer_id, CONCAT(first_name, ' ', last_name) full_name
FROM customer
WHERE active = 1;
...
SELECT address, COALESCE(postal_code, '(prazdne)') PSC
FROM address;

--Pro všechny uzavřené půjčky vypište v jednom sloupci interval od - do (t.obě data oddělená pomlčkou), kdy výpůjčka probíhala
SELECT rental_id, CONCAT(rental_date, ' - ' , return_date) interval
FROM rental
WHERE return_date IS NOT NULL;
SELECT rental_id, CAST(rental_date AS VARCHAR) + ' - ' + CAST(return_date AS VARCHAR) interval
FROM rental
WHERE return_date IS NOT NULL;
...
SELECT rental_id, CAST(rental_date AS VARCHAR) + COALESCE(CONCAT(' - ', return_date), '') interval
FROM rental;
SELECT rental_id, CAST(rental_date AS VARCHAR) + COALESCE(' - ' + CAST(return_date AS VARCHAR), '') interval
FROM rental;

--Vypište počet filmů v databází
SELECT COUNT(*) pocet_filmu
FROM film;

--Vypište počet různých klasifikací filmů (atribut rating)
SELECT COUNT(distinct rating) pocet_hodnoceni
FROM film;

--Vypište jedním dotazem počet adres, počet adres s vyplněným PSČ a počet různých PSČ
SELECT COUNT(*) pocet_adres,  COUNT(postal_code) pocet_adres_s_psc, COUNT(distinct postal_code) ruzne_psc
FROM address;
...
SELECT MIN(length) nejkrasi, MAX(length) nejdelsi, AVG(CAST(length AS FLOAT)) prumerna
FROM film;

--Vypište počet a součet všech plateb, které byly provedeny v roce 2005
SELECT COUNT(*) pocet, SUM(amount) soucet
FROM payment
WHERE YEAR(payment_date) = '2005';

--Vypište celkový počet znaků v názvech všech filmů
SELECT SUM(LEN(title)) pocet_znaku_vsech_filmu
FROM film;